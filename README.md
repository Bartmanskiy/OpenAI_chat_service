...
Повертає інформацію про session, повну історію повідомлень, загальну кількість використаних токенів та накопичену вартість.

### GET /sessions/{session_id}/messages

Повертає повідомлення конкретної session у хронологічному порядку.
...
`sessions` містить інформацію про chat session та вибрану OpenAI-модель.
...
Перед кожним запитом до OpenAI сервіс отримує попередні повідомлення конкретної session, сортує їх за часом та передає їх разом із новим повідомленням користувача.
...
* `429` — перевищено доступну OpenAI API quota або rate limit;
